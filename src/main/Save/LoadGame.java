...
public class LoadGame {
    public static LoadedFile load(String path, boolean loadGrid, boolean loadPlayers, boolean loadTowns, boolean loadUnits) throws IOException {
        ArrayList<ArrayList<Cell>> grid = null;
        Player me = null, bot = null;
        Town myTown = null, botTown = null;
        ArrayList<IUnit> myUnits = null, botUnits = null;
        ArrayList<NewUnit> unitTypes = null;
        if (loadGrid) {
            grid = loadGrid(path);
        }
        if (loadPlayers) {
            me = loadPlayer(path, "me");
            bot = loadPlayer(path, "bot");
        }
        if (loadTowns) {
            myTown = loadTown(path, "my", me);
            botTown = loadTown(path, "bot", bot);
        }
        if (loadUnits) {
            myUnits = loadUnits(path, "my", me);
            botUnits = loadUnits(path, "bot", bot);
            unitTypes = loadUnitTypes(path);
        }
        return new LoadedFile(grid, me, bot, myTown, botTown, myUnits, botUnits, unitTypes);
    }

    public static ArrayList<ArrayList<Cell>> loadGrid(String path) throws IOException {
